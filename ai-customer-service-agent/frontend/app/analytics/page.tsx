...
import { fetchAnalyticsOverview, fetchFrequentIssues } from "@/lib/api";
import type { StatsOverview, FrequentIssues } from "@/lib/types";
import { CATEGORY_LABELS } from "@/lib/constants";

export default function AnalyticsPage() {
  const [overview, setOverview] = useState<StatsOverview | null>(null);
  const [frequent, setFrequent] = useState<FrequentIssues | null>(null);
  ...
  useEffect(() => {
    Promise.all([
      ...
      .catch(() => setError("加载数据分析失败，请确认后端已启动"))
    ...
    <div className="max-w-5xl mx-auto px-4 py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          ...
          {frequent && frequent.categories.length > 0 ? (
        ...
        {/* Keywords (frequent issue keywords) */}
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          <h2 className="text-base font-semibold text-gray-900 mb-3">高频关键词</h2>
          {frequent && frequent.keywords.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {frequent.keywords.map((kw, i) => (
                <span
                  key={i}
            ...
            <p className="text-sm text-gray-400">暂无高频关键词数据</p>
          )}
        </div>

        {/* Knowledge Gaps */}
        <div className="bg-white border border-gray-200 rounded-xl p-5 md:col-span-2">
          <h2 className="text-base font-semibold text-gray-900 mb-3">知识库缺口建议</h2>
          {frequent && frequent.suggested_kb_gaps.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {frequent.suggested_kb_gaps.map((g, i) => (
